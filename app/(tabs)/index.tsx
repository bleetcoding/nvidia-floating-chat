...
export default function HomeScreen() {
  ...
  useEffect(() => {
    Promise.all([loadProviderSettings(), loadApiKey(), loadConversations(), floatingBubble.isEnabled(), floatingBubble.getAppearance(), aiKeyboard.isEnabled(), aiKeyboard.isSelected(), floatingBubble.isAccessibilityEnabled()])
      .then(([savedSettings, savedKey, savedConversations, savedBubbleEnabled, savedBubbleAppearance, savedKeyboardEnabled, savedKeyboardSelected, savedAccessibilityEnabled]) => {
  ...
  const saveSettings = async () => {
    ...
    await Promise.all([saveProviderSettings(normalized), saveApiKey(apiKey), syncKeyboardConfiguration(normalized, apiKey), floatingBubble.updateOverlayPreferences({ title: overlayTitle, excerpt: overlayExcerpt, personality: normalized.assistantPersonality ?? defaultProviderSettings.assistantPersonality ?? "", contextEnabled: Boolean(normalized.overlayContextEnabled), notificationAssistantEnabled: Boolean(normalized.notificationAssistantEnabled), voiceEnabled: Boolean(normalized.overlayVoiceEnabled), panelHeightDp: normalized.overlayPanelHeightDp ?? 380, panelWidthDp: normalized.overlayPanelWidthDp ?? 344, panelOpacity: normalized.overlayOpacity ?? .96, perAppLayoutsEnabled: Boolean(normalized.perAppLayoutsEnabled), contextExclusionsEnabled: Boolean(normalized.contextExclusionsEnabled), contextExcludedPackages: normalized.contextExcludedPackages ?? "" })]);
  ...
  const openKeyboardSettings = async () => {
    ...
    setTimeout(() => { void Promise.all([aiKeyboard.isEnabled(), aiKeyboard.isSelected()]).then(([enabled, selected]) => { setKeyboardEnabled(enabled); setKeyboardSelected(selected); }); }, 1200);
  };
  ...
  const openKeyboardPicker = async () => {
    ...
    await aiKeyboard.openPicker();
    setTimeout(() => { void aiKeyboard.isSelected().then(setKeyboardSelected); }, 1200);
  };
  ...
  const changeNotificationAssistant = async (enabled: boolean) => {
    if (enabled && Platform.OS === "android") {
      const granted = await PermissionsAndroid.request(PermissionsAndroid.PERMISSIONS.POST_NOTIFICATIONS, { title: "Allow Floating AI replies", message: "Floating AI can show an expandable notification only after stable permitted screen text is found. You can leave or reply from that notification.", buttonPositive: "Allow", buttonNegative: "Not now" });
      if (granted !== PermissionsAndroid.RESULTS.GRANTED) {
        Alert.alert("Notifications not enabled", "The screen-context assistant stays off until Android notification permission is allowed.");
        return;
      }
      if (!settings.overlayContextEnabled) await changeContextCapture(true);
    }
    setSettings((current) => ({ ...current, notificationAssistantEnabled: enabled }));
  };
  ...
  return (
    <ScreenContainer containerClassName="bg-background" className="px-5" edges={["top", "left", "right", "bottom"]}>
      ...
      {screen === "settings" ? <SettingsScreen settings={settings} apiKey={apiKey} showKey={showKey} isTesting={isTesting} result={testResult} isTestingModel={isTestingModel} modelTestResult={modelTestResult} discoveredModels={discoveredModels} keyboardEnabled={keyboardEnabled} keyboardSelected={keyboardSelected} accessibilityEnabled={accessibilityEnabled} onBack={() => setScreen("library")} onSettingsChange={updateSettings} onApiKeyChange={setApiKey} onShowKey={() => setShowKey((current) => !current)} onTest={() => void runTest()} onTestModel={() => void runSelectedModelTest()} onKeyboardSettings={() => void openKeyboardSettings()} onKeyboardPicker={() => void openKeyboardPicker()} onContextCaptureChange={(enabled) => void changeContextCapture(enabled)} onNotificationAssistantChange={(enabled) => void changeNotificationAssistant(enabled)} onVoiceChange={(enabled) => void changeVoice(enabled)} onSave={() => void saveSettings()} /> : null}
...
function SettingsScreen({ settings, apiKey, showKey, isTesting, result, isTestingModel, modelTestResult, discoveredModels, keyboardEnabled, keyboardSelected, accessibilityEnabled, onBack, onSettingsChange, onApiKeyChange, onShowKey, onTest, onTestModel, onKeyboardSettings, onKeyboardPicker, onContextCaptureChange, onNotificationAssistantChange, onVoiceChange, onSave }: { settings: ProviderSettings; apiKey: string; showKey: boolean; isTesting: boolean; result: ProviderTestResult | null; isTestingModel: boolean; modelTestResult: SelectedModelTestResult | null; discoveredModels: string[]; keyboardEnabled: boolean; keyboardSelected: boolean; accessibilityEnabled: boolean; onBack: () => void; onSettingsChange: (settings: ProviderSettings) => void; onApiKeyChange: (apiKey: string) => void; onShowKey: () => void; onTest: () => void; onTestModel: () => void; onKeyboardSettings: () => void; onKeyboardPicker: () => void; onContextCaptureChange: (enabled: boolean) => void; onNotificationAssistantChange: (enabled: boolean) => void; onVoiceChange: (enabled: boolean) => void; onSave: () => void }) {
  ...
  return <View style={styles.flex}>
    ...
    <FlatList data={["settings"]} keyExtractor={(item) => item} showsVerticalScrollIndicator={false} renderItem={() => <View style={styles.settingsContent}>
      ...
      <View style={keyboardStyles.card}><View style={keyboardStyles.heading}><View style={styles.flex}><Text style={styles.privacyTitle}>Floating AI Keyboard</Text><Text style={styles.privacyText}>{keyboardSelected ? "Floating AI Keyboard is currently selected. Open any chat field to use the six AI tools above the keys." : keyboardEnabled ? "Floating AI Keyboard is enabled, but Gboard is still selected. Tap Switch from Gboard below while a text field is open, then select Floating AI Keyboard." : "First enable Floating AI Keyboard in Android settings. Android does not allow apps to silently replace your selected keyboard."}</Text></View><View style={[keyboardStyles.status, keyboardSelected && keyboardStyles.statusOn]}><Text style={keyboardStyles.statusText}>{keyboardSelected ? "ACTIVE" : keyboardEnabled ? "SWITCH NOW" : "SET UP"}</Text></View></View><View style={keyboardStyles.toolsPreview}><Text style={keyboardStyles.toolsPreviewText}>TRANSLATE</Text><Text style={keyboardStyles.toolsPreviewText}>REWRITE</Text><Text style={keyboardStyles.toolsPreviewText}>REPLY</Text><Text style={keyboardStyles.toolsPreviewText}>GRAMMAR</Text><Text style={keyboardStyles.toolsPreviewText}>UNDO</Text><Text style={keyboardStyles.toolsPreviewText}>REDO</Text></View><TouchableOpacity accessibilityLabel="Open Android keyboard settings" onPress={onKeyboardSettings} style={keyboardStyles.button}><Text style={keyboardStyles.buttonText}>1. Enable Floating AI Keyboard</Text></TouchableOpacity><TouchableOpacity accessibilityLabel="Switch from Gboard to Floating AI Keyboard" onPress={onKeyboardPicker} style={keyboardStyles.button}><Text style={keyboardStyles.buttonText}>2. Switch from Gboard now</Text></TouchableOpacity></View>
      ...
      <View style={overlaySettingsStyles.card}>
        ...
        <View style={overlaySettingsStyles.settingRow}><View style={styles.flex}><Text style={overlaySettingsStyles.rowTitle}>Screen-context notification assistant</Text><Text style={overlaySettingsStyles.rowDetail}>Off by default. After stable permitted text, it can send a short private model response as an expandable notification with Leave and Respond actions. This needs visible screen text enabled above.</Text></View><TouchableOpacity accessibilityLabel="Toggle screen-context notification assistant" onPress={() => onNotificationAssistantChange(!settings.notificationAssistantEnabled)} style={[overlaySettingsStyles.toggle, settings.notificationAssistantEnabled && overlaySettingsStyles.toggleOn]}><View style={[overlaySettingsStyles.knob, settings.notificationAssistantEnabled && overlaySettingsStyles.knobOn]} /></TouchableOpacity></View>
        <View style={overlaySettingsStyles.settingRow}><View style={styles.flex}><Text style={overlaySettingsStyles.rowTitle}>Vision-ready image attachments</Text><Text style={overlaySettingsStyles.rowDetail}>Attach an image in a normal chat to send that selected image to a compatible vision model. Automatic screen assistance stays text-only; it does not capture screenshots.</Text></View></View>
        ...
        <Text style={overlaySettingsStyles.label}>WINDOW OPACITY · {Math.round((settings.overlayOpacity ?? .96) * 100)}%</Text><Slider accessibilityLabel="Floating window opacity" minimumValue={.55} maximumValue={1} step={.05} value={settings.overlayOpacity ?? .96} minimumTrackTintColor={colors.primary} maximumTrackTintColor={colors.border} thumbTintColor={colors.lime} onValueChange={(overlayOpacity) => onSettingsChange({ ...settings, overlayOpacity })} /><Text style={styles.helperText}>Lower opacity keeps more of the app visible behind the floating panel.</Text>
        <View style={overlaySettingsStyles.settingRow}><View style={styles.flex}><Text style={overlaySettingsStyles.rowTitle}>Remember layout for each app</Text><Text style={overlaySettingsStyles.rowDetail}>Off by default. When on, dragging or resizing in Manus, Instagram, or another app saves a separate panel size and edge position for that app.</Text></View><TouchableOpacity accessibilityLabel="Toggle per-app floating layouts" onPress={() => onSettingsChange({ ...settings, perAppLayoutsEnabled: !settings.perAppLayoutsEnabled })} style={[overlaySettingsStyles.toggle, settings.perAppLayoutsEnabled && overlaySettingsStyles.toggleOn]}><View style={[overlaySettingsStyles.knob, settings.perAppLayoutsEnabled && overlaySettingsStyles.knobOn]} /></TouchableOpacity></View>
      ...
      <View style={styles.privacyCard}><Text style={styles.privacyTitle}>Privacy checkpoint</Text><Text style={styles.privacyText}>Messages and spoken prompts are sent only after your action. If visible text context is enabled, the keyboard can send permitted text after the selected stable delay or when you tap Refresh suggestions. When the separate notification assistant is enabled, it can also send stable permitted text to create its expandable notification and keep a short local reply memory. Passwords, sensitive nodes, and any package IDs on your enabled exclusion list are excluded. Automatic context never captures screenshots; vision uses only images you intentionally attach in chat.</Text></View>
...
const keyboardStyles = StyleSheet.create({
  ...
  toolsPreview: { backgroundColor: "#1B3016", borderColor: "#426B2B", borderRadius: 10, borderWidth: 1, flexDirection: "row", justifyContent: "space-around", marginTop: 13, paddingVertical: 9 },
  toolsPreviewText: { color: colors.lime, fontSize: 10, fontWeight: "900", letterSpacing: .4 },
});
